...
func g(key [10]byte, k int, w uint16) uint16 {
...
func crypt32(key [10]byte, buf uint32, encrypt bool) uint32 {
	var kstep, k int
	...
	return uint32(byte(wr>>8))<<24 |
		uint32(byte(wr&0xFF))<<16 |
		uint32(byte(wl>>8))<<8 |
		uint32(byte(wl&0xFF))
}
...
func (s Skip32) Encrypt(id uint32) uint32 {
	return crypt32(s, id, true /* encrypt */)
}

func (s Skip32) EncryptBytes(buf []byte) {
	id := (*uint32)(unsafe.Pointer(&buf[0]))
	*id = crypt32(s, *id, true /* encrypt */)
}

func (s Skip32) Decrypt(id uint32) uint32 {
	return crypt32(s, id, false /* decrypt */)
}

func (s Skip32) DecryptBytes(buf []byte) {
	id := (*uint32)(unsafe.Pointer(&buf[0]))
	*id = crypt32(s, *id, false)
}

func ReadSkip32Key(v string) (s Skip32) {
	copy(s[:], v)
	return
}
