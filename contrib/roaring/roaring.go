...
import (
	...
	"github.com/coyove/sdss/contrib/roaring/internal"
)
...
func (ii *intIterator) Seek(minval uint32) {
	to := minval & 0xffff0000

	ii.pos = sort.Search(ii.highlowcontainer.size(), func(i int) bool {
		return uint32(ii.highlowcontainer.getKeyAtIndex(i))<<16 >= to
	})
	ii.init()
